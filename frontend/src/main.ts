...
import { invalidateCache } from "./apiClient";
...
function validate(): boolean {
  ...
  if (!author || author.length < 2) {
    ui.showFieldError("author", "Мінімум 2 символи");
    valid = false;
  }
  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    ui.showFieldError("email", "Невірний формат email");
    valid = false;
  }
...
async function handleSubmit(e: Event): Promise<void> {
  ...
  try {
    if (editId) {
      ...
      const newUser = await api.createUser({ name: author, email });
...
function checkFormFilled(): void {
  const filled =
    ...
    authorInput.value.trim().length >= 2 &&
...
[subjectInput, messageInput, authorInput, emailInput].forEach((el) =>
